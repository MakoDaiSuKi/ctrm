...
public class Position4BrokerApiController {
	...
	public ActionResult<List<Position4Broker>> Pager(HttpServletRequest request, @RequestBody PositionParams param) {
		try {
			...
			// 买卖
			if (!StringUtils.isBlank(param.getLS())) {
				criteria.add(Restrictions.eq("LS", param.getLS()));
			}
			
			// 开平方向
			if (!StringUtils.isBlank(param.getOCS())) {
	...
	@RequestMapping("Pager4Select")
	@ResponseBody
	public ActionResult<List<Position4Broker>> Pager4Select(HttpServletRequest request, @RequestBody PositionParams param) {
		try {
			if (param == null) {
				param = new PositionParams();
			}
			...
			if (!StringUtils.isBlank(param.getOCS())) {
				...
				criteria.add(Restrictions.or(
						Restrictions.eq("OCS", "S"),
						Restrictions.and(
								Restrictions.eq("OCS", "C"),
								Restrictions.le("TradeDate", DateUtil.endOfTodDay(param.getTradeStartDate())),
								Restrictions.ge("TradeDate", DateUtil.startOfTodDay(param.getTradeStartDate())))
						));
			}
	...
	public ActionResult<String> Save(HttpServletRequest request, @RequestBody Position4Broker position) {
		try {
			...
			ActionResult<Position4Broker> result = position4BrokerService.Save(position);
	 ...
	 * @return
	 */
	@RequestMapping("Save4Return")
	@ResponseBody
	public ActionResult<Position4Broker> Save4Return(HttpServletRequest request, @RequestBody Position4Broker position) {
		try {
			if (position.getId() != null) {
				position.setUpdatedId(LoginHelper.GetLoginInfo(request).getUserId());
			} else {
	 ...
	 * @return
	 */
	@RequestMapping("Unravel")
	@ResponseBody
	public ActionResult<String> Unravel(HttpServletRequest request,
			@RequestBody Position4BrokerParams pos4BParams) {
		try {
			return position4BrokerService.Unravel(pos4BParams);
		} catch (Exception ex) {
	...
	@ResponseBody
	public ActionResult<String> ChangeMonth(HttpServletRequest request,
			@RequestBody Position4BrokerParams pos4BParams) {
		try {
			return position4BrokerService.ChangeMonth(pos4BParams);
		} catch (Exception ex) {
			logger.error(ex.getMessage(), ex);
			return new ActionResult<>(Boolean.FALSE, ex.getMessage());
		}
